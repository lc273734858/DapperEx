...
namespace FWW.Framework.DapperEx
{
        ...
        where T : BaseEntity
    {
        ...
        public FromSection(Database database, bool withNolock = false)
            : this(database, null, withNolock)
        {
        ...
        /// <param name="withNolock"></param>
        public FromSection(Database database, DbTransaction trans, bool withNolock = false)
            : base(database, database.DbProvider.BuildTableName(EntityCache.GetTableName<T>()), trans, withNolock)
        {
        ...
        /// <returns></returns>
        public FromSection<T> RightJoin<TEntity>(Expression<Func<T, TEntity, bool>> lambdaWhere)
            where TEntity : class
        {
            return join(EntityCache.GetTableName<TEntity>(), ExpressionToClip<T>.ToJoinWhere(lambdaWhere), JoinType.RightJoin);
        }
        /// <summary>
        ...
        {
            //CacheInfo.PrimiaryKey;
            Field[] primarykeys = CacheInfo.Fields.ToArray();
            ...
            {
                temporderBy = new OrderByClip(CacheInfo.GetFirstField());
            }
        ...
        public List<TResult> ToList<TResult>()
        {
            ...
            {
                return database.GetConnection().Query<TResult>(from.SqlString, from.Parameters).ToList();
            }
            if (!@from.Fields.Any())
            {
        ...
        {
            FromSection from = getPagedFromSection();
            return database.GetConnection().Query<T>(from.SqlString, from.Parameters).ToList();
        }
        /// <summary>
        /// 返回懒加载数据
        /// </summary>
        ...
        {
            FromSection from = getPagedFromSection();
            return database.GetConnection().Query<T>(from.SqlString, from.Parameters);
        }
        /// <summary>
        ...
        public TResult ToFirst<TResult>() where TResult : class
        {
            ...
            var list = database.GetConnection().Query<TResult>(from.SqlString, from.Parameters);
            return list.First();
        }
        /// <summary>
        /// 返回第一个实体 ，同First()。无数据返回Null。
        ...
        {
            FromSection from = this.Top(1).getPagedFromSection();
            return database.GetConnection().Query<T>(from.SqlString, from.Parameters).First();
        }
        ...
        public new FromSection<T> Union(FromSection fromSection)
        {
            ...
            tmpfromSection.parameters.Concat(this.Parameters);
            tmpfromSection.parameters.Concat(fromSection.Parameters);
    ...
    public class FromSection
    {
        ...
        */
        ...
        /// 记录数sql语句 count
        /// </summary>
        protected string CountSqlString
        {
            get
            {
        ...
        internal string FromString
        {
            get
            {
                ...
                {
                    fromstring.Append(dbProvider.GenerateFromTableName(tableName, _withnolock));
                    foreach (KeyValuePair<string, KeyValuePair<string, WhereClip>> kv in joins)
                    {
                        fromstring.Append(" ");
                        fromstring.Append(kv.Value.Key);
                        fromstring.Append(" ");
                        fromstring.Append(dbProvider.GenerateFromTableName(kv.Key, _withnolock));
                        fromstring.Append(" ON ");
                        fromstring.Append(kv.Value.Value.GenerateExpress(dataParameter));
                    }
                }
        ...
        public string SqlString
        {
            get
            {
                ...
                if (!WhereClip.IsNullOrEmpty(where))
                {
                    var wheresql = where.GenerateExpress(dataParameter);
                ...
                database.LastSqlExpress = sqlstr;
                database.LastSqlParameters = dataParameter;
                return sqlstr;
            }
        }
        ...
        /// </summary>
        public SqlDataParameter Parameters
        {
            get
            {
                ...
                //    ps.Concat(havingWhere.Parameters);

                //ps.AddRange(parameters);
                //database.LastSqlParameters = ps;
                //return ps;
        ...
        internal EntityCacheInfo CacheInfo;

        protected SqlDataParameter dataParameter;
        #endregion
        ...
        public FromSection(Database database, string tableName, DbTransaction trans, bool withNolock = false)
        {
            Check.NotNullOrEmpty(database, "database");
            Check.NotNullOrEmpty(tableName, "tableName");
            dataParameter = new SqlDataParameter();
        ...
        */
        ...
        protected string formatSql(string sql, FromSection from)
        {
            ...
            SqlDataParameter listPara = from.Parameters;
            foreach (var p in listPara)
            {
                tempSql = tempSql.Replace(p.Key, p.Value == null ? string.Empty : p.Value.ToString());
            }
            return tempSql;
        }
        ...
        {
            return database.GetConnection().ExecuteScalar<int>(from.CountSqlString, from.Parameters, trans, null, null);
        }
        ...
        {
            FromSection from = getPagedFromSection();
            return database.ExecuteDataSet(createDbCommand(from), trans);
        }
        ...
        protected DbCommand createDbCommand(FromSection from)
        {
            ...
            database.AddCommandParameter(dbCommand, from.Parameters);
            return dbCommand;
        }
        ...
        public object ToScalar()
        {
            ...
            FromSection from = getPagedFromSection();
            return database.GetConnection().ExecuteScalar(from.SqlString, from.Parameters, trans, null, null);
        }
        ...
        protected FromSection join(string tableName, WhereClip where, JoinType joinType)
        {
            ...
            if (!joins.ContainsKey(tableName))
            {
                ...
                if (where.Parameters.Count > 0)
                    parameters.Concat(where.Parameters);
            }
        ...
        public FromSection Union(FromSection fromSection)
        {
            ...
            tmpfromSection.CacheInfo = this.CacheInfo;
